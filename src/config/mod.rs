...
//! `DATABASE_URL` lives in `~/.superwave/.env` (loaded via dotenvy early
...
impl Config {
    ...
    ///
    /// Loads both `./.env` (standard, higher priority) and `~/.superwave/.env`
    ...
    /// If `None`, tries the default path `~/.superwave/config.toml` (missing
