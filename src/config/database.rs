...
pub struct DatabaseConfig {
    ...
    // -- libSQL fields --
    /// Path to local libSQL database file (default: ~/.superwave/superwave.db).
...
impl DatabaseConfig {
    pub(crate) fn resolve() -> Result<Self, ConfigError> {
        ...
        // DATABASE_URL is loaded from ~/.superwave/.env via dotenvy early in startup.
        let url = optional_env("DATABASE_URL")?
            ...
            .ok_or_else(|| ConfigError::MissingRequired {
                ...
                hint: "Run 'superwave-agent onboard' or set DATABASE_URL environment variable".to_string(),
            })?;
...
/// Default libSQL database path (~/.superwave/superwave.db).
pub fn default_libsql_path() -> PathBuf {
    ironclaw_base_dir().join("superwave.db")
}
