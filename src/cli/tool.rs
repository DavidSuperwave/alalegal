...
async fn list_tools(dir: Option<PathBuf>, verbose: bool) -> anyhow::Result<()> {
    ...
    if !tools_dir.exists() {
        ...
        println!("Install a tool with: superwave-agent tool install <path>");
        return Ok(());
    }
    ...
    let setup = caps.setup.ok_or_else(|| {
        anyhow::anyhow!(
            "Tool '{}' has no setup configuration.\n\
             ...
             Try 'superwave-agent tool auth {}' for OAuth-based authentication.",
            name,
            name
        )
    })?;
...
mod tests {
    ...
    fn test_default_tools_dir() {
        ...
        assert!(dir.to_string_lossy().contains(".superwave"));
