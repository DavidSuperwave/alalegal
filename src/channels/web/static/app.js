// Superwave Agent Web Gateway - Client
...
function authenticate() {
  ...
  apiFetch('/api/chat/threads')
    .then(() => {
      sessionStorage.setItem('superwave_token', token);
    ...
    .catch(() => {
      sessionStorage.removeItem('superwave_token');
...
(function autoAuth() {
  ...
  const saved = sessionStorage.getItem('superwave_token');
  if (saved) {
...
function restartGateway() {
  if (!confirm('Restart Superwave gateway? Active connections will be dropped.')) return;
...
function searchSkillsHub() {
  ...
  apiFetch('/api/skills/search', {
    ...
    if (data.catalog_error) {
      ...
      warning.textContent = 'Could not reach Skills registry: ' + data.catalog_error;
...
function renderCatalogSkillCard(entry, installedNames) {
  ...
  if (isInstalled) {
    ...
    installBtn.addEventListener('click', (function(s, btn) {
      return function() {
        if (!confirm('Install skill "' + s + '" from Skills Hub?')) return;
        btn.disabled = true;
...
document.getElementById('skill-search-input').addEventListener('keydown', function(e) {
  if (e.key === 'Enter') searchSkillsHub();
});
