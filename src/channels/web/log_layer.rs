...
impl LogLevelHandle {
    ...
    pub fn set_level(&self, level: &str) -> Result<(), String> {
        ...
        let filter_str = if self.base_filter.is_empty() {
            format!("superwave_agent={}", level)
        } else {
            format!("superwave_agent={},{}", level, self.base_filter)
        };
    ...
    /// Returns the current superwave_agent log level (e.g. "info", "debug").
...
pub fn init_tracing(log_broadcaster: Arc<LogBroadcaster>) -> Arc<LogLevelHandle> {
    let raw_filter =
        std::env::var("RUST_LOG").unwrap_or_else(|_| "superwave_agent=info,tower_http=warn".to_string());

    // Split into the ironclaw directive and "everything else" (base_filter).
    let mut agent_level = String::from("info");
    let mut base_parts: Vec<&str> = Vec::new();

    for part in raw_filter.split(',') {
        let trimmed = part.trim();
        if trimmed.starts_with("superwave_agent=") {
            if let Some(lvl) = trimmed.strip_prefix("superwave_agent=") {
                agent_level = lvl.to_string();
            }
...
///   INFO superwave_agent::agent: Request completed url="http://..." status=200
///
...
mod tests {
    ...
    fn test_log_broadcaster_subscribe() {
        ...
        broadcaster.send(LogEntry {
            ...
            target: "superwave_agent::test".to_string(),
    ...
    fn test_log_entry_serialization() {
        let entry = LogEntry {
            ...
            target: "superwave_agent::agent".to_string(),
