...
//!
//! Stores user preferences in ~/.superwave/settings.json (or legacy ~/.openclaw).
...
fn default_sandbox_image() -> String {
    "superwave-worker:latest".to_string()
}
...
impl Settings {
    ...
    /// Get the default settings file path (`<base>/settings.json`).
    ...
    /// Default TOML config file path (`<base>/config.toml`).
    ...
    pub fn save_toml(&self, path: &std::path::Path) -> Result<(), String> {
        ...
        let content = format!(
            "# Superwave Agent configuration file.\n\
             #\n\
             # Priority: env var > this file > database settings > defaults.\n\
             # Uncomment and edit values to override defaults.\n\
             # Run `superwave-agent config init` to regenerate this file.\n\
             #\n\
             # Documentation: https://github.com/DavidSuperwave/superwave-agent\n\
             \n\
             {raw}"
        );
...
mod tests {
    ...
    fn test_get_setting() {
        ...
        assert_eq!(settings.get("agent.name"), Some("superwave".to_string()));
        assert_eq!(
    ...
    fn test_reset_setting() {
        ...
        assert_eq!(settings.agent.name, "superwave");
    }
    ...
    fn toml_header_comment_present() {
        ...
        assert!(content.starts_with("# Superwave Agent configuration file."));
    ...
    #[test]
    fn default_toml_path_under_superwave_or_legacy_openclaw() {
        let path = Settings::default_toml_path();
        let path_str = path.to_string_lossy();
        assert!(path_str.contains(".superwave") || path_str.contains(".openclaw"));
    ...
    fn wizard_recovery_step1_overrides_stale_db() {
        ...
        let prior_run = Settings {
            ...
            database_url: Some("postgres://old-host/superwave".to_string()),
            llm_backend: Some("anthropic".to_string()),
        ...
        let step1_settings = Settings {
            database_backend: Some("postgres".to_string()),
            database_url: Some("postgres://new-host/superwave".to_string()),
            ..Settings::default()
        };
        ...
        assert_eq!(
            ...
            Some("postgres://new-host/superwave".to_string()),
