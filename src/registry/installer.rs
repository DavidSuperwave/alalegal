...
impl RegistryInstaller {
    ...
    pub async fn install_from_source(
        ...
        // CLI commands (`superwave-agent tool auth <name>`) all agree on the stem.
    ...
    pub async fn install_bundle(
        ...
        if let Some(shared) = &bundle.shared_auth {
            auth_hints.push(format!(
                "Bundle uses shared auth '{}'. Run `superwave-agent tool auth <any-member>` to authenticate all members.",
                shared
            ));
        }
...
mod tests {
    ...
    fn test_installer_creation() {
        let installer = RegistryInstaller::new(
            PathBuf::from("/repo"),
            PathBuf::from("/home/.superwave/tools"),
            PathBuf::from("/home/.superwave/channels"),
        );
