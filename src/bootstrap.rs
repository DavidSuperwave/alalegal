...
//!
//! File: `~/.superwave/.env` (standard dotenvy format)

use std::path::PathBuf;
use std::sync::LazyLock;

const SUPERWAVE_BASE_DIR_ENV: &str = "SUPERWAVE_BASE_DIR";
const LEGACY_IRONCLAW_BASE_DIR_ENV: &str = "IRONCLAW_BASE_DIR";
const LEGACY_OPENCLAW_STATE_DIR_ENV: &str = "OPENCLAW_STATE_DIR";
const SUPERWAVE_DIRNAME: &str = ".superwave";
const LEGACY_OPENCLAW_DIRNAME: &str = ".openclaw";
    ...
    explicit_base_dir_override()
        ...
        .map(|path| {
            if path.as_os_str().is_empty() {
                ...
                eprintln!(
                    "Warning: SUPERWAVE_BASE_DIR/IRONCLAW_BASE_DIR is a relative path '{}', resolved against current directory",
                    path.display()
                );
                path
            } else {
...
fn explicit_base_dir_override() -> Result<String, std::env::VarError> {
    std::env::var(SUPERWAVE_BASE_DIR_ENV)
        .or_else(|_| std::env::var(LEGACY_IRONCLAW_BASE_DIR_ENV))
        .or_else(|_| std::env::var(LEGACY_OPENCLAW_STATE_DIR_ENV))
}

/// Get the default IronClaw base directory (~/.superwave).
///
/// Logs a warning if the home directory cannot be determined and falls back to
/// the current directory.
fn default_base_dir() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        let superwave_dir = home.join(SUPERWAVE_DIRNAME);
        if superwave_dir.exists() {
            return superwave_dir;
        }

        let legacy_dir = home.join(LEGACY_OPENCLAW_DIRNAME);
        ...
        std::env::current_dir()
            ...
            .join(SUPERWAVE_DIRNAME)
    }
}

/// Get the IronClaw base directory.
///
/// Override with `SUPERWAVE_BASE_DIR` (or legacy `IRONCLAW_BASE_DIR`) environment variable.
/// Defaults to `~/.superwave` (or `./.superwave` if home directory cannot be determined).
///
/// Thread-safe: the value is computed once and cached in a `LazyLock`.
///
/// # Environment Variable Behavior
/// - If `SUPERWAVE_BASE_DIR` (or legacy `IRONCLAW_BASE_DIR`) is set to a non-empty path, that path is used.
/// - If override env contains an empty string, it is treated as unset.
/// - If override env contains null bytes, a warning is printed and the default is used.
...
/// Path to the IronClaw-specific `.env` file: `~/.superwave/.env`.
pub fn ironclaw_env_path() -> PathBuf {
    ironclaw_base_dir().join(".env")
}

/// Load env vars from `~/.superwave/.env` (in addition to the standard `.env`).
///
/// Call this **after** `dotenvy::dotenv()` so that the standard `./.env`
/// takes priority over `~/.superwave/.env`. dotenvy never overwrites
/// existing env vars, so the effective priority is:
///
///   explicit env vars > `./.env` > `~/.superwave/.env` > auto-detect
///
/// If `~/.superwave/.env` doesn't exist but the legacy `bootstrap.json` does,
...
/// `DATABASE_BACKEND` is still unset and `~/.superwave/superwave.db` exists,
...
pub fn load_ironclaw_env() {
    ...
    // all env files, and the local SQLite DB exists under the active base dir,
    // default to libsql.
    // This avoids the chicken-and-egg problem on cloud instances where no
    // DATABASE_URL is configured but superwave.db is already present.
    if std::env::var("DATABASE_BACKEND").is_err() {
        let default_db = ironclaw_base_dir().join("superwave.db");
...
/// Write database bootstrap vars to `~/.superwave/.env`.
///
...
/// Update or add a single variable in `~/.superwave/.env`, preserving existing content.
///
...
/// Write `DATABASE_URL` to `~/.superwave/.env`.
///
...
/// One-time migration of legacy `~/.superwave/settings.json` into the database.
///
...
pub async fn migrate_disk_to_db(
    ...
    // 2. Write DATABASE_URL to ~/.superwave/.env
...
mod tests {
    ...
    fn test_save_and_load_database_url() {
        ...
        let url = "postgres://localhost:5432/superwave_test";
        ...
        assert_eq!(
            content,
            "DATABASE_URL=\"postgres://localhost:5432/superwave_test\"\n"
        );
...
INJECTED="pwned"#;
    ...
    fn test_ironclaw_env_path() {
        let path = ironclaw_env_path();
        assert!(path.ends_with(".superwave/.env"));
    }
    ...
    fn test_migrate_bootstrap_json_to_env() {
        ...
        let bootstrap_json = serde_json::json!({
            "database_url": "postgres://localhost/superwave_upgrade",
    ...
    fn test_save_bootstrap_env_multiple_vars() {
        ...
        let vars = [
            ...
            ("LIBSQL_PATH", "/home/user/.superwave/superwave.db"),
        ];
        ...
        assert_eq!(
            ...
            (
                ...
                "/home/user/.superwave/superwave.db".to_string()
            )
        );
    }
    ...
    fn test_libsql_autodetect_sets_backend_when_db_exists() {
        ...
        let dir = tempdir().unwrap();
        let db_path = dir.path().join("superwave.db");
    ...
    fn test_ironclaw_base_dir_default() {
        ...
        // It verifies that when base-dir overrides are not set, the default path is used.
        let _guard = ENV_MUTEX.lock().unwrap();
        let old_superwave = std::env::var("SUPERWAVE_BASE_DIR").ok();
        let old_ironclaw = std::env::var("IRONCLAW_BASE_DIR").ok();
        let old_openclaw_state = std::env::var("OPENCLAW_STATE_DIR").ok();
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::remove_var("SUPERWAVE_BASE_DIR") };
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::remove_var("IRONCLAW_BASE_DIR") };
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::remove_var("OPENCLAW_STATE_DIR") };

        // Force re-evaluation by calling the computation function directly
        let path = compute_ironclaw_base_dir();
        let home = dirs::home_dir().unwrap_or_else(|| std::path::PathBuf::from("."));
        assert!(
            path == home.join(".superwave") || path == home.join(".openclaw"),
            "unexpected default base dir: {}",
            path.display()
        );

        if let Some(val) = old_superwave {
            // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
            unsafe { std::env::set_var("SUPERWAVE_BASE_DIR", val) };
        } else {
            // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
            unsafe { std::env::remove_var("SUPERWAVE_BASE_DIR") };
        }
        ...
        if let Some(val) = old_openclaw_state {
            // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
            unsafe { std::env::set_var("OPENCLAW_STATE_DIR", val) };
        } else {
            // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
            unsafe { std::env::remove_var("OPENCLAW_STATE_DIR") };
        }
    ...
    fn test_ironclaw_base_dir_env_override() {
        ...
        let _guard = ENV_MUTEX.lock().unwrap();
        let old_val = std::env::var("SUPERWAVE_BASE_DIR").ok();
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::set_var("SUPERWAVE_BASE_DIR", "/custom/superwave/path") };

        // Force re-evaluation by calling the computation function directly
        let path = compute_ironclaw_base_dir();
        assert_eq!(path, std::path::PathBuf::from("/custom/superwave/path"));
    ...
    fn test_compute_base_dir_env_path_join() {
        ...
        let old_val = std::env::var("SUPERWAVE_BASE_DIR").ok();
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::set_var("SUPERWAVE_BASE_DIR", "/my/custom/dir") };
    ...
    fn test_ironclaw_base_dir_empty_env() {
        ...
        let old_openclaw_state = std::env::var("OPENCLAW_STATE_DIR").ok();
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::set_var("SUPERWAVE_BASE_DIR", "") };
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::remove_var("IRONCLAW_BASE_DIR") };
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::remove_var("OPENCLAW_STATE_DIR") };
    ...
    fn test_ironclaw_base_dir_special_chars() {
        ...
        let old_val = std::env::var("SUPERWAVE_BASE_DIR").ok();
        // SAFETY: ENV_MUTEX ensures single-threaded access to env vars in tests
        unsafe { std::env::set_var("SUPERWAVE_BASE_DIR", "/tmp/test_with-special.chars") };
