...
//! - **macOS**: launchd plist at `~/Library/LaunchAgents/com.superwave.daemon.plist`
//! - **Linux**: systemd user unit at `~/.config/systemd/user/superwave-agent.service`
//!
//! The installed service runs `superwave-agent run` (the default agent mode) and is
...
const SERVICE_LABEL: &str = "com.superwave.daemon";
const SYSTEMD_UNIT: &str = "superwave-agent.service";
...
"#,
    ...
    println!("  Start with: superwave-agent.service start");
    Ok(())
}
...
fn install_linux() -> Result<()> {
    ...
    let unit = format!(
        "[Unit]\n\
         Description=Superwave Agent daemon\n\
...
fn start() -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        if !plist.exists() {
            bail!("Service not installed. Run `superwave-agent.service install` first.");
        }
...
mod tests {
    ...
    fn macos_plist_path_has_expected_suffix() {
        ...
        assert!(
            s.ends_with("Library/LaunchAgents/com.superwave.daemon.plist"),
            "unexpected path: {s}"
        );
    }
    ...
    fn linux_unit_path_has_expected_suffix() {
        ...
        let s = path.to_string_lossy();
        assert!(
            s.ends_with(".config/systemd/user/superwave-agent.service"),
            "unexpected path: {s}"
        );
    }
    ...
    fn logs_dir_under_ironclaw() {
        ...
        let s = path.to_string_lossy();
        assert!(s.ends_with(".superwave/logs"), "unexpected path: {s}");
    }
}
