...
use superwave_agent::setup::{SetupConfig, SetupWizard};
...
fn main() -> anyhow::Result<()> {
    ...
    superwave_agent::bootstrap::load_ironclaw_env();
...
async fn async_main() -> anyhow::Result<()> {
    ...
    match &cli.command {
        ...
        Some(Command::Config(config_cmd)) => {
            init_cli_tracing();
            return superwave_agent::cli::run_config_command(config_cmd.clone()).await;
        }
        Some(Command::Registry(registry_cmd)) => {
            init_cli_tracing();
            return superwave_agent::cli::run_registry_command(registry_cmd.clone()).await;
        }
        ...
        Some(Command::Doctor) => {
            init_cli_tracing();
            return superwave_agent::cli::run_doctor_command().await;
        }
        Some(Command::Status) => {
            init_cli_tracing();
    ...
    let config = match Config::from_env_with_toml(toml_path).await {
        Ok(c) => c,
        Err(superwave_agent::error::ConfigError::MissingRequired { key, hint }) => {
            eprintln!("Configuration error: Missing required setting '{}'", key);
            eprintln!("  {}", hint);
            eprintln!();
            eprintln!(
                "Run 'superwave-agent onboard' to configure, or set the required environment variables."
            );
            std::process::exit(1);
        }
    ...
    let log_level_handle =
        superwave_agent::channels::web::log_layer::init_tracing(Arc::clone(&log_broadcaster));

    tracing::info!("Starting Superwave Agent...");
    ...
    if config.llm.backend == superwave_agent::config::LlmBackend::NearAi
    ...
    let docker_status = if config.sandbox.enabled {
        let detection = superwave_agent::sandbox::check_docker().await;
        match detection.status {
            superwave_agent::sandbox::DockerStatus::Available => {
                tracing::info!("Docker is available");
            }
            superwave_agent::sandbox::DockerStatus::NotInstalled => {
                tracing::warn!(
                    "Docker is not installed -- sandbox disabled for this session. {}",
                    detection.platform.install_hint()
                );
            }
            superwave_agent::sandbox::DockerStatus::NotRunning => {
                tracing::warn!(
                    "Docker is installed but not running -- sandbox disabled for this session. {}",
                    detection.platform.start_hint()
                );
            }
            superwave_agent::sandbox::DockerStatus::Disabled => {}
        }
        detection.status
    } else {
        superwave_agent::sandbox::DockerStatus::Disabled
    ...
    let job_event_tx: Option<
        tokio::sync::broadcast::Sender<(uuid::Uuid, superwave_agent::channels::web::types::SseEvent)>,
    ...
    let prompt_queue = Arc::new(tokio::sync::Mutex::new(std::collections::HashMap::<
        uuid::Uuid,
        std::collections::VecDeque<superwave_agent::orchestrator::api::PendingPrompt>,
    >::new()));
    ...
    let container_job_manager: Option<Arc<ContainerJobManager>> =
        if config.sandbox.enabled && docker_status.is_ok() {
            ...
            let job_config = ContainerJobConfig {
                ...
                claude_code_oauth_token: superwave_agent::config::ClaudeCodeConfig::extract_oauth_token(),
    ...
    let session_manager =
        Arc::new(superwave_agent::agent::SessionManager::new().with_hooks(components.hooks.clone()));
    ...
    let scheduler_slot: superwave_agent::tools::builtin::SchedulerSlot =
    ...
    let mut sse_sender: Option<
        tokio::sync::broadcast::Sender<superwave_agent::channels::web::types::SseEvent>,
    > = None;
    let mut gateway_state: Option<std::sync::Arc<superwave_agent::channels::web::server::GatewayState>> =
        None;
    ...
    if config.channels.cli.enabled && cli.message.is_none() {
        let boot_info = superwave_agent::boot_screen::BootInfo {
        ...
        superwave_agent::boot_screen::print_boot_screen(&boot_info);
    }
    ...
    *scheduler_slot.write().await = Some(agent.scheduler());
    ...
    {
        eprintln!("Restarting Superwave Agent (exit code 75)...");
        std::process::exit(75);
    }
...
fn init_worker_tracing() {
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("superwave_agent=info")),
        )
        .init();
}

/// Run the Memory CLI subcommand.
async fn run_memory_command(mem_cmd: &superwave_agent::cli::MemoryCommand) -> anyhow::Result<()> {
    ...
    if config.database.backend == superwave_agent::config::DatabaseBackend::LibSql
    ...
    let db: Arc<dyn superwave_agent::db::Database> = superwave_agent::db::connect_from_config(&config.database)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e))?;

    superwave_agent::cli::run_memory_command_with_db(mem_cmd.clone(), db, embeddings).await
}
...
async fn run_worker(
    ...
    let config = superwave_agent::worker::runtime::WorkerConfig {
        job_id,
    ...
    let runtime = superwave_agent::worker::WorkerRuntime::new(config)
...
async fn run_claude_bridge(
    ...
    let config = superwave_agent::worker::claude_bridge::ClaudeBridgeConfig {
        job_id,
        orchestrator_url: orchestrator_url.to_string(),
        max_turns,
        model: model.to_string(),
        timeout: std::time::Duration::from_secs(1800),
        allowed_tools: superwave_agent::config::ClaudeCodeConfig::from_env().allowed_tools,
    };

    let runtime = superwave_agent::worker::ClaudeBridgeRuntime::new(config)
...
async fn start_tunnel(
    mut config: superwave_agent::config::Config,
) -> (
    superwave_agent::config::Config,
    Option<Box<dyn superwave_agent::tunnel::Tunnel>>,
) {
    ...
    match superwave_agent::tunnel::create_tunnel(provider_config) {
        Ok(Some(tunnel)) => {
            tracing::info!(
                "Starting {} tunnel on {}:{}...",
                tunnel.name(),
...
struct WasmChannelSetup {
    channels: Vec<(String, Box<dyn superwave_agent::channels::Channel>)>,
...
async fn setup_wasm_channels(
    config: &superwave_agent::config::Config,
    secrets_store: &Option<Arc<dyn SecretsStore + Send + Sync>>,
    extension_manager: Option<&Arc<superwave_agent::extensions::ExtensionManager>>,
    database: Option<&Arc<dyn superwave_agent::db::Database>>,
    ...
    let settings_store: Option<Arc<dyn superwave_agent::db::SettingsStore>> =
        database.map(|db| Arc::clone(db) as Arc<dyn superwave_agent::db::SettingsStore>);
    ...
    let mut channels: Vec<(String, Box<dyn superwave_agent::channels::Channel>)> = Vec::new();
    let mut channel_names: Vec<String> = Vec::new();
...
fn check_onboard_needed() -> Option<&'static str> {
    let has_db = std::env::var("DATABASE_URL").is_ok()
        ...
        || superwave_agent::config::default_libsql_path().exists();
    ...
    if std::env::var("NEARAI_API_KEY").is_err() {
        let session_path = superwave_agent::llm::session::default_session_path();
...
async fn inject_channel_credentials(
    channel: &Arc<superwave_agent::channels::wasm::WasmChannel>,
