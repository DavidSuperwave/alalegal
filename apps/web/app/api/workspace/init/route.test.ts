...
describe("POST /api/workspace/init", () => {
  ...
  const STATE_DIR = join("/home/testuser", ".superwave");
  ...
  it("handles custom workspace path", async () => {
    ...
    expect(response.status).toBe(200);
    expect(mockMkdir).toHaveBeenCalledWith(resolve("/my/custom/workspace"), {
      recursive: true,
    });
    const json = await response.json();
    expect(json.workspaceDir).toBe(resolve("/my/custom/workspace"));
  });
  ...
  it("resolves tilde in custom path", async () => {
    ...
    expect(mockMkdir).toHaveBeenCalledWith(
      resolve(join("/home/testuser", "my-workspace")),
      { recursive: true },
    );
  });
