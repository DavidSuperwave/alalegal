...
const PRIMARY_STATE_DIRNAME = ".superwave";
const LEGACY_STATE_DIRNAME = ".openclaw";
const UI_STATE_FILENAME = ".superwave-ui-state.json";
const LEGACY_UI_STATE_FILENAME = ".ironclaw-ui-state.json";
...
// stateDir/.superwave-ui-state.json so it survives server restarts.
// ---------------------------------------------------------------------------
...
type UIState = {
  activeProfile?: string | null;
	/** Maps profile names to absolute workspace paths for workspaces outside stateDir. */
  workspaceRegistry?: Record<string, string>;
};

function expandHome(pathValue: string): string {
	return pathValue.startsWith("~") ? join(homedir(), pathValue.slice(1)) : pathValue;
}

function resolveStateDirOverride(): string | null {
	const directOverride = process.env.SUPERWAVE_STATE_DIR?.trim();
	if (directOverride) {return expandHome(directOverride);}
	const legacyOverride = process.env.OPENCLAW_STATE_DIR?.trim();
	if (legacyOverride) {return expandHome(legacyOverride);}
	return null;
}

function resolveStateHomeDir(): string {
	return (
		process.env.SUPERWAVE_HOME?.trim() ||
		process.env.OPENCLAW_HOME?.trim() ||
		homedir()
	);
}

function resolvePreferredStateDir(): string {
	return join(resolveStateHomeDir(), PRIMARY_STATE_DIRNAME);
}

function resolveLegacyStateDir(): string {
	return join(resolveStateHomeDir(), LEGACY_STATE_DIRNAME);
}
...
function legacyUiStatePath(): string {
	return join(resolveOpenClawStateDir(), LEGACY_UI_STATE_FILENAME);
}

function hasStateConfig(stateDir: string): boolean {
	return (
		existsSync(join(stateDir, "superwave.json")) ||
		existsSync(join(stateDir, "openclaw.json"))
	);
}

function readUIState(): UIState {
	const candidates = [uiStatePath(), legacyUiStatePath()];
...
// ---------------------------------------------------------------------------
// Workspace registry — remembers workspaces created outside stateDir.
// ---------------------------------------------------------------------------
...
/**
 * Discover all profiles by scanning stateDir for workspace-* directories
 * and checking for profile-specific state dirs.
 */
export function discoverProfiles(): DiscoveredProfile[] {
	const baseStateDir = resolveOpenClawStateDir();
  ...
  profiles.push({
    ...
    isActive: !activeProfile || activeProfile.toLowerCase() === "default",
		hasConfig: hasStateConfig(baseStateDir),
  });
  seen.add("default");
  ...
  // Merge workspaces registered via custom paths (outside the default state dir)
...
/**
 * Resolve the state directory (base dir for config, sessions, agents, etc.).
 * Mirrors src/config/paths.ts:resolveStateDir() logic for the web app.
 *
 * Precedence:
 * 1. SUPERWAVE_STATE_DIR env var
 * 2. OPENCLAW_STATE_DIR env var (legacy)
 * 3. SUPERWAVE_HOME env var → <home>/.superwave
 * 4. OPENCLAW_HOME env var (legacy) → <home>/.openclaw
 * 5. Auto-migrate behavior:
 *    - use existing ~/.superwave when present
 *    - else use existing ~/.openclaw when present
 *    - else default to ~/.superwave for fresh installs
 */
export function resolveOpenClawStateDir(): string {
	const override = resolveStateDirOverride();
	if (override) {
		return override;
  }
	const preferred = resolvePreferredStateDir();
	if (existsSync(preferred)) {return preferred;}
	const legacy = resolveLegacyStateDir();
 ...
 * Returns the absolute workspace path (e.g. ~/.superwave/workspace),
 ...
 * e.g. for `~/.superwave/workspace/dench/workspace.duckdb` returns `"dench"`.
