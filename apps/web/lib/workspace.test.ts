...
describe("workspace utilities", () => {
  ...
  describe("resolveWorkspaceRoot", () => {
    ...
    it("returns default ~/.superwave/workspace when env not set", async () => {
      delete process.env.OPENCLAW_WORKSPACE;
      const { resolveWorkspaceRoot, mockExists } = await importWorkspace();
      const defaultPath = join("/home/testuser", ".superwave", "workspace");
  ...
  describe("resolveAgentWorkspacePrefix", () => {
    ...
    it("returns absolute path when workspace is outside repo", async () => {
      process.env.OPENCLAW_WORKSPACE = abs("/external/workspace");
      const { resolveAgentWorkspacePrefix, mockExists } = await importWorkspace();
      mockExists.mockImplementation((p) => String(p) === abs("/external/workspace"));
      vi.spyOn(process, "cwd").mockReturnValue(abs("/repo/apps/web"));
      expect(resolveAgentWorkspacePrefix()).toBe(abs("/external/workspace"));
    });

    it("returns relative path when workspace is inside repo", async () => {
      process.env.OPENCLAW_WORKSPACE = abs("/repo/workspace");
      const { resolveAgentWorkspacePrefix, mockExists } = await importWorkspace();
      mockExists.mockImplementation((p) => String(p) === abs("/repo/workspace"));
      vi.spyOn(process, "cwd").mockReturnValue(abs("/repo/apps/web"));
    ...
    it("handles non apps/web cwd", async () => {
      ...
      const { resolveAgentWorkspacePrefix, mockExists } = await importWorkspace();
      mockExists.mockImplementation((p) => String(p) === abs("/repo/workspace"));
      vi.spyOn(process, "cwd").mockReturnValue(abs("/repo"));
      expect(resolveAgentWorkspacePrefix()).toBe("workspace");
    });
  });
  ...
  describe("duckdbPath", () => {
    ...
    it("returns null when workspace exists but no duckdb files", async () => {
      process.env.OPENCLAW_WORKSPACE = "/ws";
      const { duckdbPath, mockExists, mockReaddir } = await importWorkspace();
      mockExists.mockImplementation((p) => String(p) === abs("/ws"));
      mockReaddir.mockReturnValue([]);
      expect(duckdbPath()).toBeNull();
    });
  });
  ...
  describe("safeResolvePath", () => {
    it("resolves valid path within workspace", async () => {
      process.env.OPENCLAW_WORKSPACE = abs("/ws");
      const { safeResolvePath, mockExists } = await importWorkspace();
      mockExists.mockImplementation((p) => {
        const s = String(p);
        return s === abs("/ws") || s === abs("/ws/knowledge/doc.md");
      });
      expect(safeResolvePath("knowledge/doc.md")).toBe(abs("/ws/knowledge/doc.md"));
    });
  ...
  describe("safeResolveNewPath", () => {
    it("resolves valid new path (does not require existence)", async () => {
      ...
      mockExists.mockImplementation((p) => String(p) === abs("/ws"));
      expect(safeResolveNewPath("new-folder/file.txt")).toBe(abs("/ws/new-folder/file.txt"));
    });
    ...
    it("handles deeply nested new paths", async () => {
      ...
      const { safeResolveNewPath, mockExists } = await importWorkspace();
      mockExists.mockImplementation((p) => String(p) === abs("/ws"));
      expect(safeResolveNewPath("a/b/c/d/e.txt")).toBe(abs("/ws/a/b/c/d/e.txt"));
    });
  });
  ...
  describe("readWorkspaceFile", () => {
    it("reads markdown file and detects type", async () => {
      ...
      mockExists.mockImplementation((p) => {
        const s = String(p);
        return s === abs("/ws") || s === abs("/ws/doc.md");
      });
    ...
    it("reads yaml file and detects type", async () => {
      ...
      mockExists.mockImplementation((p) => {
        const s = String(p);
        return s === abs("/ws") || s === abs("/ws/config.yaml");
      });
    ...
    it("reads yml file as yaml type", async () => {
      ...
      mockExists.mockImplementation((p) => {
        const s = String(p);
        return s === abs("/ws") || s === abs("/ws/config.yml");
      });
      mockReadFile.mockReturnValue("key: value" as never);
    ...
    it("reads text file with generic type", async () => {
      ...
      mockExists.mockImplementation((p) => {
        const s = String(p);
        return s === abs("/ws") || s === abs("/ws/notes.txt");
      });
