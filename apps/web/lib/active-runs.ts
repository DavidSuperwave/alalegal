...
export function sendSubagentFollowUp(sessionKey: string, message: string): boolean {
	try {
		...
		const child = spawn(
			cli.command,
			[
				...cli.argsPrefix, "gateway", "call", "agent",
...
function extractLegacyCliText(rawStdout: string): string | null {
	...
	const jsonChunk = trimmed.slice(jsonStart, jsonEnd + 1);
...
function wireChildProcess(run: ActiveRun): void {
	...
	child.on("close", (code) => {
		...
		if (!everSentText) {
			const legacyText = extractLegacyCliText(stdoutChunks.join(""));
			if (legacyText) {
				const tid = nextId("text");
				emit({ type: "text-start", id: tid });
				emit({ type: "text-delta", id: tid, delta: legacyText });
				emit({ type: "text-end", id: tid });
				accAppendText(legacyText);
				everSentText = true;
			}
		}
