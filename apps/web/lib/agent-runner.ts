...
function executableExistsInPath(candidate: string): boolean {
	...
	for (const dir of entries) {
		for (const ext of extCandidates) {
			const suffix = ext && !candidate.toLowerCase().endsWith(ext.toLowerCase()) ? ext : "";
			const fullPath = join(dir, `${candidate}${suffix}`);
...
/**
 * Resolve how to invoke the agent CLI in this environment.
 *
 * Priority order:
 * 1) Dev launcher script (`scripts/run-node.mjs`)
 * 2) Production launcher script (`openclaw.mjs`)
 * 3) Native CLI binary on PATH (`superwave-agent`, then legacy aliases)
 */
export function resolveAgentCliCommand(root: string): CliCommand {
	const devScript = join(root, "scripts", "run-node.mjs");
	if (existsSync(devScript)) {
		return { command: "node", argsPrefix: [devScript] };
	}

	const prodScript = join(root, "openclaw.mjs");
	if (existsSync(prodScript)) {
		return { command: "node", argsPrefix: [prodScript] };
	}

	const explicitCliBin = process.env.SUPERWAVE_CLI_BIN?.trim();
	const candidates = [
		explicitCliBin,
		"superwave-agent",
		"openclaw",
	...
	for (const candidate of candidates) {
		// Allow absolute/relative paths from SUPERWAVE_CLI_BIN.
		if (candidate.includes("/") || candidate.includes("\\") || extname(candidate)) {
			if (existsSync(candidate)) {
				// Windows npm shims (*.cmd/*.bat) can mangle argument quoting for
				// long messages. Prefer calling the underlying JS entrypoint via node.
				if (process.platform === "win32" && /\.(cmd|bat)$/i.test(candidate)) {
					const shimDir = dirname(candidate);
					const openclawMjs = join(shimDir, "node_modules", "openclaw", "openclaw.mjs");
					const ironclawMjs = join(shimDir, "node_modules", "ironclaw", "ironclaw.mjs");
					if (existsSync(openclawMjs)) {
						return { command: "node", argsPrefix: [openclawMjs] };
					}
					if (existsSync(ironclawMjs)) {
						return { command: "node", argsPrefix: [ironclawMjs] };
					}
				}
		...
		if (executableExistsInPath(candidate)) {
			return { command: candidate, argsPrefix: [] };
		}
	}

	// Last-resort fallback; spawn() will surface ENOENT if unavailable.
	return { command: "superwave-agent", argsPrefix: [] };
}
...
export function spawnAgentProcess(
	...
	const cli = resolveAgentCliCommand(root);
	const requiresShell =
	...
	const isLegacyOpenclawCli =
		isLegacyScript ||
		cliBase === "openclaw" ||
		cliBase === "openclaw.cmd" ||
		cliBase === "openclaw.exe" ||
		cliBase === "ironclaw" ||
